...
fun PopularMovieListScreen(
    ...
    if (movieListUiState.popularMovieList.isEmpty()) {
        ...
        LazyVerticalGrid(
            ...
            items(movieListUiState.popularMovieList.size) { movie ->
                MovieItem(
                    movie = movieListUiState.popularMovieList[movie],
                    navHostController = navHostController,
                )

                Spacer(modifier = Modifier.height(16.dp))

                if(movieListUiState.popularMovieList.size - 1 == movie && !movieListUiState.isLoading){
